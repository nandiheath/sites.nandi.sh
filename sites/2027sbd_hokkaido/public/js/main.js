...
function renderPrepare(){
  document.querySelector('#prepare').innerHTML=`<div class="section-intro"><div><p class="eyebrow">07 / PRE-DEPARTURE</p><h2 class="section-title">出發前</h2><p class="muted">個人清單，只存於這個瀏覽器。</p></div><button type="button" class="button secondary" id="print-plan">列印本頁 ↗</button></div><div class="split-panel preparation-layout"><article class="card"><div class="card-top"><h3>個人清單</h3><span id="packing-count" class="tag" aria-live="polite"></span></div><progress id="packing-progress" max="${packing.length}" value="0" aria-label="準備進度"></progress><div class="packing-list">${packing.map(([id,label,note])=>`<label class="packing-item"><input type="checkbox" data-pack="${id}" ${checklist[id]===true?'checked':''}><span><strong>${label}</strong><small>${note}</small></span></label>`).join('')}</div><p class="meta" id="storage-note">${storageAvailable?'本機儲存，不同步團友或公開快照。':'瀏覽器不能儲存；關閉後不保留勾選。'}</p></article><aside class="stack"><article class="card warm-card"><p class="eyebrow">TO CONFIRM</p><h3>全團待辦</h3><ol class="numbered-list"><li>覆核 ${members.length} 人名單、未確認出席、兩間酒店及分房。</li><li>收齊去回航班，才訂接送時間。</li><li>包車報價：人數、板袋、行李、動物園等候。</li><li>確認新季巴士、單板課程、票價及食店訂位。</li><li>出發前一週、48 小時及當日核對天氣／營運。</li></ol><a href="#members" class="text-link">團友狀態及後續跟進計劃 ↗</a></article><article class="card emergency-card"><p class="eyebrow">EMERGENCY / JAPAN</p><div class="emergency-numbers"><a href="tel:119"><strong>119</strong><span>救護・消防</span></a><a href="tel:110"><strong>110</strong><span>警察</span></a></div><p>雪場內先找巡邏隊，講清雪道名、最近標示及位置。</p>${link('https://www.japan.travel/en/plan/hotline/','JNTO 旅客支援')}</article></aside></div><article class="card budget-card"><div><p class="eyebrow">TRANSFER COST</p><h3>包車分攤試算</h3><p class="muted">原構想粗估 ¥195,000–220,000，未有正式報價。只計輸入的包車費，不是旅費總額；預設最多 ${members.length} 人，可按最終人數更改。</p></div><div class="budget-controls"><label for="charter-total">全團包車費（JPY）<input id="charter-total" type="number" min="0" max="10000000" step="1000" value="210000"></label><label for="charter-people">分攤人數<input id="charter-people" type="number" min="1" max="50" step="1" value="${members.length}"></label><output id="budget-result" aria-live="polite"></output></div></article>`;
