...
const days = [
  {date:'03.06',weekday:'SAT',place:'富良野',type:'抵達',title:'新千歲 → 富良野',hotel:'新富良野王子酒店',location:'Shin Furano Prince Hotel',steps:[['抵達','機場集合','取齊行李及板袋，再到已約定的上車點。航班時間待補。'],['接送','包車到酒店','預約包車優先；巴士要查 2026–27 班次及雪袋限額。'],['晚上','Check-in／試裝備','取單板、試雪鞋，確認明天教練及集合點。有時間才去 Ningle Terrace。']],note:'今天不排上雪。先確認實際人數、航班和板袋數量。'},
  ...
  {date:'03.09',weekday:'TUE',place:'旭川',type:'REST / TRANSFER',title:'富良野 → 動物園 → OMO7',hotel:'OMO7 旭川',location:'Asahiyama Zoo',steps:[['上午','退房／包車出發','箱和板袋留車安排、等候及停車費預先講清楚。'],['日間','旭山動物園','冬季 10:30–15:30，最後入園 15:00；企鵝散步看當日公告。'],['傍晚','OMO7 Check-in','市區食拉麵。確認明天雪場來回巴士和裝備。']],note:'整天休息，不加夜滑。動物園營業資料已公布，仍留意臨時改動。'},
  ...
  {date:'03.12',weekday:'FRI',place:'神居／旭岳',type:'SNOWBOARD 05',title:'最後雪日 · 按雪況決定',hotel:'OMO7 旭川',location:'Asahidake Ropeway',steps:[['早上','查雪況及風','一般組神居／比布。旭岳只限能力、嚮導及安全條件齊備的高手組。'],['日間','最後一天上雪','不合條件就改普通雪場。旭岳風停、白茫及雪崩風險不能靠買票解決。'],['晚上','最後一餐／還板','提前訂位、執行李，核對機場接送和航班。']],note:'旭岳不是一般單板雪場；不安排新手或普通中級團員滑行。'},
  {date:'03.13',weekday:'SAT',place:'旭川 → CTS',type:'RETURN',title:'退房 → 新千歲機場',hotel:'退房 · 不留宿',location:'New Chitose Airport',steps:[['出發前','按航班倒推','先預留冬季交通及雪袋寄艙時間；早機可能要改前一晚住宿。'],['接送','包車／JR／公共機場巴士','JR 經札幌。現行 Taisetsu Liner 停 OMO7 前，2027 班次及板袋承運待確認。'],['機場','先寄艙，再購物','有剩餘時間才食飯或買手信；不要用轉車緩衝去購物。']],note:'航班未齊，暫不寫死出門時間。交通頁有機場路線和預約連結。'},
];
const sections = [['journey','行程'],['transport','交通'],['mountains','雪場・雪票'],['explore','食嘢・去邊'],['stays','酒店'],['prepare','出發前']];
...
<div class="trip-stats"><div><span class="stat-label">DATES / 日期</span><strong>06—13 <small>MAR</small></strong><span>2027 · 8 日 7 晚</span></div><div><span class="stat-label">ON SNOW / 上雪</span><strong>05 <small>DAYS</small></strong><span>單板為主 · 3/9 休息</span></div><div><span class="stat-label">GROUP / 人數</span><strong>12 <small>人目標</small></strong><span>實際人數另行確認</span></div><div><span class="stat-label">BASES / 住宿</span><strong>03 <small>+</small> 04 <small>NIGHTS</small></strong><span>富良野王子 → OMO7</span></div></div>
<section id="day-planner" class="section-block"><div class="section-intro"><div><p class="eyebrow">01 / DAILY PLAN</p><h2 class="section-title">8 日行程</h2><p class="muted">選日期看安排。3/11–12 按雪況、程度及交通分組。</p></div><button type="button" class="button secondary" id="calendar-download">下載行事曆 <span aria-hidden="true">↓</span></button></div><div class="day-strip" role="group" aria-label="選擇行程日期">${days.map((d,i)=>`<button type="button" class="day-button" data-day="${i}" aria-pressed="${i===0}" aria-controls="day-detail"><span>DAY 0${i+1}<small>${d.weekday}</small></span><strong>${d.date}</strong><em>${d.place}</em><i class="day-dot ${i===3?'rest':''}"></i></button>`).join('')}</div><div class="itinerary-layout"><article id="day-detail" class="day-detail" aria-live="polite"></article><aside class="journey-map"><p class="eyebrow">TRIP ROUTE / 非比例示意</p><h3>兩個基地，五日上雪</h3><svg viewBox="0 0 380 255" role="img" aria-label="新千歲至富良野，經旭山動物園至旭川，再返新千歲"><defs><linearGradient id="map-fill" x2="1" y2="1"><stop stop-color="#1c4260"/><stop offset="1" stop-color="#0e2339"/></linearGradient></defs><path d="m165 12 39 23 12 27 43 20 22 38 46 6-17 34-34 22-8 41-43-8-22 27-38-16-20-29-37 4-39-26 33-29-5-43 44-23Z" fill="url(#map-fill)" stroke="#29516f"/><path class="route-trace" d="M132 214Q226 195 216 150T197 79l48 17" stroke="#71d7ff" stroke-width="2" stroke-dasharray="5 6" fill="none"/><g fill="#81ddff"><circle cx="132" cy="214" r="5"/><circle cx="216" cy="150" r="5"/><circle cx="197" cy="79" r="5"/><circle cx="245" cy="96" r="4"/></g><text x="60" y="239">新千歲 CTS</text><text x="232" y="157">富良野</text><text x="131" y="68">旭川</text><text x="258" y="89">動物園</text><text x="38" y="42" class="map-water">N ↑</text></svg><div class="map-legend"><span><i></i>王子 · 3 晚</span><span><i></i>OMO7 · 4 晚</span></div><button type="button" class="text-link" data-goto="transport">交通及訂車 ↗</button></aside></div><div class="editor-note"><span>PLAN STATUS</span><p>公開行程摘要；航班、包車及課程需另行確認。此頁不提供個別訂房紀錄。</p><button type="button" class="text-link" data-goto="stays">住宿安排 ↗</button></div></section>
</section>
<section id="transport" class="page-section module-section" hidden></section><section id="mountains" class="page-section module-section" hidden></section><section id="explore" class="page-section module-section" hidden></section><section id="stays" class="page-section module-section" hidden></section><section id="prepare" class="page-section module-section" hidden></section>
</main><footer class="site-footer"><a href="#journey" class="footer-brand">SBD / HOKKAIDO 2027</a><p>公開行程版 · 資料核對 2026.09.04 · 日本時間 UTC+9</p><p>舊季價格已標示；出發前再核對班次及營業。</p><details><summary>資料及分享</summary><p>已移除團友名稱、個別訂房紀錄及私人文件連結；行程日期與酒店安排為公開資料。這是人工整理的摘要，不會自動更新。個人清單只存於本機。</p></details></footer><div id="toast" role="status" aria-live="polite"></div>`;
...
function renderStays(){
  document.querySelector('#stays').innerHTML=`
    <div class="section-intro"><div><p class="eyebrow">05 / BASES</p><h2 class="section-title">酒店</h2><p class="muted">富良野住新富良野王子酒店 3 晚，旭川住 OMO7 4 晚。只列住宿安排，不公開個別訂房紀錄。</p></div><span class="tag">7 NIGHTS</span></div>
    <div class="card-grid two hotel-grid">
      ...
      <article class="card hotel-card">
        ...
        <div class="hotel-body"><span class="tag green">03.09 入住 → 03.13 退房 · 4 晚</span><h3>OMO7 旭川</h3>
          ...
          <details><summary>機場交通／未確認事項</summary><p>現行 Taisetsu Liner 公共機場巴士停酒店前，不是酒店 shuttle。2027 班次及板袋承運要重查；包車或 JR 經札幌是其他選項。</p><p>房型、早餐及取消條款請直接向酒店確認。</p>${link('https://www.asahikawa-denkikidou.jp/taisetsu_liner/','機場巴士官方')}</details>
        </div>
      </article>
    </div>`;
}
renderStays();
const packing=[['passport','護照・機票','核對入境要求、雪袋寄艙額及重量。'],['insurance','單板保險','醫療、雪場救援；旭岳／野雪要另查保障。'],['rooms','酒店訂單','早餐、取消期、分房和私人聯絡方式。'],['transport','來回交通','3/6、3/9、3/13 包車；每日雪場返程。'],['lesson','教練・單板租借','Snowboard 班、教學語言、程度、鞋碼及站姿。'],['layers','上雪裝備','頭盔、雪鏡、手套、排汗層、雪褸褲及雪襪。'],['street','防滑鞋・防曬','防水鞋、合適防滑配件、暖包及防曬。'],['mobile','上網・充電','eSIM／漫遊、充電器、外置充電器、離線地圖。'],['health','藥物・過敏','私人告知主辦人，不放公開網站。'],['cash','日圓現金','小店、巴士、儲物櫃可能只收現金。']];
...
function renderPrepare(){
  document.querySelector('#prepare').innerHTML=`<div class="section-intro"><div><p class="eyebrow">06 / PRE-DEPARTURE</p><h2 class="section-title">出發前</h2><p class="muted">個人清單，只存於這個瀏覽器。</p></div><button type="button" class="button secondary" id="print-plan">列印本頁 ↗</button></div><div class="split-panel preparation-layout"><article class="card"><div class="card-top"><h3>個人清單</h3><span id="packing-count" class="tag" aria-live="polite"></span></div><progress id="packing-progress" max="${packing.length}" value="0" aria-label="準備進度"></progress><div class="packing-list">${packing.map(([id,label,note])=>`<label class="packing-item"><input type="checkbox" data-pack="${id}" ${checklist[id]===true?'checked':''}><span><strong>${label}</strong><small>${note}</small></span></label>`).join('')}</div><p class="meta" id="storage-note">${storageAvailable?'本機儲存，不同步團友或原表。':'瀏覽器不能儲存；關閉後不保留勾選。'}</p></article><aside class="stack"><article class="card warm-card"><p class="eyebrow">TO CONFIRM</p><h3>全團待辦</h3><ol class="numbered-list"><li>補齊名單、出席、分房。</li><li>收齊去回航班，才訂接送時間。</li><li>包車報價：人數、板袋、行李、動物園等候。</li><li>確認新季巴士、單板課程、票價及食店訂位。</li><li>出發前一週、48 小時及當日核對天氣／營運。</li></ol></article><article class="card emergency-card"><p class="eyebrow">EMERGENCY / JAPAN</p><div class="emergency-numbers"><a href="tel:119"><strong>119</strong><span>救護・消防</span></a><a href="tel:110"><strong>110</strong><span>警察</span></a></div><p>雪場內先找巡邏隊，講清雪道名、最近標示及位置。</p>${link('https://www.japan.travel/en/plan/hotline/','JNTO 旅客支援')}</article></aside></div><article class="card budget-card"><div><p class="eyebrow">TRANSFER COST</p><h3>包車分攤試算</h3><p class="muted">原表粗估 ¥195,000–220,000，未有正式報價。只計輸入的包車費，不是旅費總額。</p></div><div class="budget-controls"><label for="charter-total">全團包車費（JPY）<input id="charter-total" type="number" min="0" max="10000000" step="1000" value="210000"></label><label for="charter-people">分攤人數<input id="charter-people" type="number" min="1" max="50" step="1" value="12"></label><output id="budget-result" aria-live="polite"></output></div></article>`;
  ...
  const calculate=()=>{const total=document.querySelector('#charter-total');const people=document.querySelector('#charter-people');const n=people.valueAsNumber;document.querySelector('#budget-result').textContent=total.validity.valid&&people.validity.valid&&Number.isFinite(total.valueAsNumber)&&Number.isInteger(n)&&n>=1?`約 ¥${Math.ceil(total.valueAsNumber/n).toLocaleString('en-US')}／人`:'請輸入有效金額及 1–50 的整數人數';};
