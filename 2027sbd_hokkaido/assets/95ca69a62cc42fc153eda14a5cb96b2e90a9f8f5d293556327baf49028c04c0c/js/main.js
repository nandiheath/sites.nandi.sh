import { renderJourneyMap, updateJourneyMap } from './journey-map.js';
...
import { renderChangelogPreview } from './changelog.js';
...
const days = [
  ...
  {date:'03.09',weekday:'TUE',place:'東川 → 旭川',type:'REST / TRANSFER',title:'富良野 → 丹丘蒸留所／自選停靠 → OMO7',hotel:'OMO7 旭川',location:'OMO7 Asahikawa',steps:[['上午','退房／Monchan Travel 巴士出發','Monchan Travel 約 ¥110,000 巴士報價；固定由新富良野王子到 OMO7，先確認車型、行李容量及停靠條款。'],['日間','丹丘蒸留所（Perfume Trees 團隊）／自選景點＋午餐','候選酒廠位於東川。現行星期二導賞是 16:00 韓語；中文／廣東話及較早私人場仍須另詢，未確認前不排死。交通頁有官方導賞來源。'],['傍晚','OMO7 Check-in','原目標約 15:00–16:00 到酒店；若要參加 16:00 酒廠團，必須先同意較晚入住及車商加時，不能兩者都承諾。']],note:'Monchan Travel 約 ¥110,000 只屬巴士報價，不是訂車確認。丹丘蒸留所是 Perfume Trees Gin 創辦團隊的東川酒廠；3/9 團體時段、中文／廣東話導賞及車輛停泊仍待確認。整天不加夜滑。'},
...
const sections = [['journey','行程'],['transport','交通'],['mountains','雪場・雪票'],['explore','食嘢・去邊'],['stays','酒店'],['members','團友清單'],['equipment','租裝備・衣物'],['prepare','出發前']];
...
<aside class="booking-alert" aria-labelledby="booking-alert-title">
  <div class="booking-alert-copy"><p class="eyebrow">BOOKING STATUS / 訂房狀態</p><h2 id="booking-alert-title">兩間酒店・團友確認快照</h2><p><strong>新富良野王子酒店 · 2027/3/6 入住 → 3/9 退房 · 3 晚 · ${bookingSummary('furano')}</strong></p><p><strong>OMO7 旭川 · 2027/3/9 入住 → 3/13 退房 · 4 晚 · ${bookingSummary('omo')}</strong></p><p>Peggy 同 Kit 共用同一房間，兩間酒店均已訂。各人狀態以團友清單的公開快照為準；請保留自己的訂單，出發前核對入住姓名、日期、早餐、稅費及取消條款。</p><p class="meta">個別訂單、房型及房號不公開；酒店卡的官方連結只供查閱，唔需要重複訂房。</p></div>
  <div class="booking-alert-action"><span class="tag">訂房快照</span><a class="text-link" href="#members">核對團友狀態 ↗</a></div>
</aside>
...
<section id="day-planner" class="section-block"><div class="section-intro"><div><p class="eyebrow">01 / DAILY PLAN</p><h2 class="section-title">8 日行程</h2><p class="muted">選日期看安排。3/11–12 按雪況、程度及交通分組。</p></div><button type="button" class="button secondary" id="calendar-download">下載行事曆 <span aria-hidden="true">↓</span></button></div><div class="day-strip" role="group" aria-label="選擇行程日期">${days.map((d,i)=>`<button type="button" class="day-button" data-day="${i}" aria-pressed="${i===0}" aria-controls="day-detail journey-map"><span>DAY 0${i+1}<small>${d.weekday}</small></span><strong>${d.date}</strong><em>${d.place}</em><i class="day-dot ${i===3?'rest':''}"></i></button>`).join('')}</div><div class="itinerary-layout"><article id="day-detail" class="day-detail" aria-live="polite"></article>${renderJourneyMap()}</div><div class="editor-note"><span>PLAN STATUS</span><p>公開行程摘要；航班、包車及課程需另行確認。出席及兩間酒店狀態見團友清單。</p><button type="button" class="text-link" data-goto="members">團友清單 ↗</button></div></section>
</section>
<section id="transport" class="page-section module-section" hidden></section><section id="mountains" class="page-section module-section" hidden></section><section id="explore" class="page-section module-section" hidden></section><section id="stays" class="page-section module-section" hidden></section><section id="members" class="page-section module-section" hidden></section><section id="equipment" class="page-section module-section" hidden></section><section id="prepare" class="page-section module-section" hidden></section>
...
renderEquipment(document.querySelector('#equipment'));
